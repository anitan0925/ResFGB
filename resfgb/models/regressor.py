...
import time
import numpy as np
from resfgb.utils import minibatches, minibatches2
...
class Regressor2(Model):
    def __init__(self, eta, scale, minibatch_size, eval_iters, seed, log_level):
        ...
        seed           : integer.
                         Seed for random module.
        """
        super(Regressor2, self).__init__(seed)
    ...
    def evaluate(self, X, Z, Y):
        n, loss = 0, 0.
        minibatch_size = np.min((10000, Z.shape[0]))
        for (Xb, Zb, Yb) in minibatches2(minibatch_size, X, Z, Y, shuffle=False):
            n_ = Zb.shape[0]
            loss_ = self.loss_func(Xb, Zb, Yb)
            loss = (n / (n + n_)) * loss + (n_ / (n + n_)) * loss_
            n += n_
    ...
    def evaluate_eta(self, X, Z, Y, eta):
        self.__save_param()
        self.optimizer.set_eta(eta)
        ...
        eval_f = True
        while eval_f:
            for (Xb, Zb, Yb) in minibatches2(self.minibatch_size, X, Z, Y, shuffle=True):
                if n_iters >= self.eval_iters:
                    eval_f = False
                    break
                self.optimizer.update_func(Xb, Zb, Yb)
                n_iters += 1

        val = self.evaluate(X, Z, Y)
        self.__load_param()
        self.optimizer.reset_func()

        return val

    def determine_eta(self, X, Z, Y, factor=2.):
        val0 = self.evaluate(X, Z, Y)

        low_eta = self.__eta
        low_val = self.evaluate_eta(X, Z, Y, low_eta)
        low_val = np.inf if np.isnan(low_val) else low_val

        high_eta = factor * low_eta
        high_val = self.evaluate_eta(X, Z, Y, high_eta)
        high_val = np.inf if np.isnan(high_val) else high_val
    ...
    def fit(self, X, Z, Y, max_epoch, early_stop=-1):
        """
        Run algorigthm for up to (max_epoch) on training data X.
        ...
        init_train_loss = self.evaluate(X, Z, Y)
        best_loss = 1e+10
        best_epoch = 0
        ...
        while success is False:
            ...
            for e in range(max_epoch):
                stime = time.time()
                for (Xb, Zb, Yb) in minibatches2(self.minibatch_size,
                                                 X, Z, Y, shuffle=True):
                    self.optimizer.update_func(Xb, Zb, Yb)
                etime = time.time()
                total_time += etime - stime
